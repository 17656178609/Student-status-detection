...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, DyHeadBlock, Segment, Pose)):
            s = 256  # 2x min stride
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, PConv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C2f_DCN, C2f_Res2Block, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, CBAM , GAM_Attention, C2f_MHSA, C2f_CloAtt):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C2f_Res2Block, C3, C3TR, C3Ghost, C3x, C2f_DCN, C2f_MHSA, C2f_CloAtt):
        ...
        elif m in {MHSA, ShuffleAttention, EMA, EfficientAttention}:
            args=[ch[f],*args]
        elif m in (Detect, Segment, Pose, DyHeadBlock):
